...
assert.match(previewViewer, /const contextPick = molstarContextPickFromEvent\(event\) \|\| molstarStandaloneContextMenuPick\(\);[\s\S]*?event\.preventDefault\(\);\s*event\.stopPropagation\(\);[\s\S]*?contextPointer = \{[\s\S]*?pick: contextPick/);
...
assert.match(previewViewer, /function molstarStandaloneContextMenuPick\(\)/);
assert.match(previewViewer, /const contextPick = molstarContextPickFromEvent\(event[^;]*\) \|\| molstarStandaloneContextMenuPick\(\);/);
assert.match(previewViewer, /if \(!picked\?\.structure\) \{\s*return \{ structures: \[\], label: activeConfig\?\.label \|\| 'Mol\* structure', scope: 'none' \};\s*\}/);
assert.match(previewViewer, /const pickedScope = resolved\.selectionBased[\s\S]*?molstarContextUsesWholeMoleculeScope\(\) \? 'ligand' : molstarContextScopeForAtom\(pickedAtom\)/);
...
assert.match(gridViewer, /selectedGraphsCompatible: selectedRowsHaveCompatibleGraphProfiles\(selectedRows\)/);
assert.match(gridViewer, /function selectedRowsHaveCompatibleGraphProfiles\(rows\)/);
assert.match(gridUi, /!props\.selectedGraphsCompatible/);
assert.match(gridUi, /same explicit atom and bond graph/);
