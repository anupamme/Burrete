...
assert.match(gridMessages, /await openTextDocuments\(\[result\.reportPath\], \{ background: true \}\)/);
assert.match(gridMessages, /gridAlignmentFinished[\s\S]*?addDocuments\(\[document\]\)/);
...
assert.match(gridViewer, /gridSemiempiricalFinished[\s\S]*?setGridViewMode\('table', config\(\)\)/);
assert.match(gridViewer, /gridAlignmentFinished[\s\S]*?setGridViewMode\('table', config\(\)\)/);
assert.match(gridUi, /id="calculate-semiempirical-selected"/);
assert.match(gridUi, /id="calculate-semiempirical-selected"[\s\S]*?disabled=\{props\.evaluatingSemiempirical \|\| !allSelectedHaveCoordinates\}/);
